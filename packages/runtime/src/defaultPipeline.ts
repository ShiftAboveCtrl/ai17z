...
export function defaultPipelineDraft(triggerLabel = 'Mention received'): PipelineDraft {
  return {
    ...
    nodes: [
      ...
      { key: 'consistency', kind: 'STANCE_CHECK', label: 'Still consistent', config: {}, x: 0, y: 12 },
      { key: 'approval', kind: 'APPROVAL_GATE', label: 'Approval gate', config: {}, x: 0, y: 13 },
      { key: 'rejected', kind: 'END', label: 'Rejected', config: { reason: 'rejected by the operator' }, x: 1, y: 14 },
      { key: 'execute', kind: 'EXECUTE_ACTION', label: 'Execute action', config: { actionType: 'REPLY' }, x: 0, y: 14 },
      { key: 'remember', kind: 'MEMORY_WRITE', label: 'Record and remember', config: {}, x: 0, y: 15 },

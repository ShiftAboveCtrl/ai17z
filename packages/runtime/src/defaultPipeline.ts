...
export function defaultPipelineDraft(triggerLabel = 'Mention received'): PipelineDraft {
  return {
    ...
    nodes: [
      ...
      { key: 'memory', kind: 'RETRIEVE_MEMORY', label: 'Retrieve memory', config: {}, x: 0, y: 3 },
      { key: 'persona', kind: 'ASSEMBLE_PERSONA', label: 'Assemble persona', config: {}, x: 0, y: 4 },
      { key: 'generate', kind: 'GENERATE', label: 'Generate', config: { role: 'primary' }, x: 0, y: 5 },
      { key: 'validate', kind: 'VALIDATE', label: 'Validate', config: {}, x: 0, y: 6 },
      { key: 'approval', kind: 'APPROVAL_GATE', label: 'Approval gate', config: {}, x: 0, y: 7 },
      { key: 'rejected', kind: 'END', label: 'Rejected', config: { reason: 'rejected by the operator' }, x: 1, y: 8 },
      { key: 'execute', kind: 'EXECUTE_ACTION', label: 'Execute action', config: { actionType: 'REPLY' }, x: 0, y: 8 },
      { key: 'remember', kind: 'MEMORY_WRITE', label: 'Record and remember', config: {}, x: 0, y: 9 },
    ...
    edges: [
      ...
      { from: 'context', to: 'memory', branch: 'next', condition: null },

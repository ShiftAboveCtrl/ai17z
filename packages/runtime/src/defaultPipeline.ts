...
export function defaultPipelineDraft(triggerLabel = 'Mention received'): PipelineDraft {
  return {
    ...
    nodes: [
      ...
      // After relationship and stance, because who is asking and what the agent
      // already thinks both change whether answering is worth it.
      { key: 'engagement', kind: 'ENGAGEMENT_DECISION', label: 'Worth answering', config: {}, x: 0, y: 6 },
      ...
      { key: 'validate', kind: 'VALIDATE', label: 'Validate', config: {}, x: 0, y: 11 },
      // After validation, so the output policy has already been applied and this
      // is only changing how it reads.
      { key: 'voice', kind: 'VOICE', label: 'Sound like itself', config: {}, x: 0, y: 12 },
      { key: 'quality', kind: 'QUALITY_GATE', label: 'Good enough to send', config: {}, x: 0, y: 13 },
      { key: 'consistency', kind: 'STANCE_CHECK', label: 'Still consistent', config: {}, x: 0, y: 14 },
      { key: 'approval', kind: 'APPROVAL_GATE', label: 'Approval gate', config: {}, x: 0, y: 15 },
      { key: 'rejected', kind: 'END', label: 'Rejected', config: { reason: 'rejected by the operator' }, x: 1, y: 16 },
      { key: 'execute', kind: 'EXECUTE_ACTION', label: 'Execute action', config: { actionType: 'REPLY' }, x: 0, y: 16 },
      { key: 'remember', kind: 'MEMORY_WRITE', label: 'Record and remember', config: {}, x: 0, y: 17 },
    ...
    edges: [
      ...
      { from: 'stance', to: 'engagement', branch: 'next', condition: null },
      ...
      { from: 'validate', to: 'voice', branch: 'next', condition: null },
      { from: 'voice', to: 'quality', branch: 'next', condition: null },
      { from: 'quality', to: 'consistency', branch: 'next', condition: null },

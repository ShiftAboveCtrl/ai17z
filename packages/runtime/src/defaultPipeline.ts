...
export function defaultPipelineDraft(triggerLabel = 'Mention received'): PipelineDraft {
  return {
    ...
    nodes: [
      ...
      { key: 'memory', kind: 'RETRIEVE_MEMORY', label: 'Retrieve memory', config: {}, x: 0, y: 5 },
      { key: 'persona', kind: 'ASSEMBLE_PERSONA', label: 'Assemble persona', config: {}, x: 0, y: 6 },
      { key: 'generate', kind: 'GENERATE', label: 'Generate', config: { role: 'primary' }, x: 0, y: 7 },
      { key: 'validate', kind: 'VALIDATE', label: 'Validate', config: {}, x: 0, y: 8 },
      { key: 'approval', kind: 'APPROVAL_GATE', label: 'Approval gate', config: {}, x: 0, y: 9 },
      { key: 'rejected', kind: 'END', label: 'Rejected', config: { reason: 'rejected by the operator' }, x: 1, y: 10 },
      { key: 'execute', kind: 'EXECUTE_ACTION', label: 'Execute action', config: { actionType: 'REPLY' }, x: 0, y: 10 },
      { key: 'remember', kind: 'MEMORY_WRITE', label: 'Record and remember', config: {}, x: 0, y: 11 },
    ...
    edges: [
      ...
      { from: 'relationship', to: 'memory', branch: 'next', condition: null },
      ...
      { from: 'validate', to: 'approval', branch: 'next', condition: null },

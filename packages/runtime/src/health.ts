/**
 * What is wrong with this installation, in one place.
 *
 * There were two answers to that question. This one, which the web Health
 * screen reads, weighs the database, the providers, the models, the channels,
 * the workers, the browser and how much memory the machine has left. And a
 * second one inside `telegramCommands.ts`, which counted workers and open
 * notifications and knew nothing about any of the rest.
 *
 * So a phone could be told everything was fine while the screen said a provider
 * had stopped answering. Two health models is one more than a person can hold,
 * and the one on the phone is the one somebody reads at three in the morning.
 *
 * It lives here rather than in the API because the API is not the only caller
 * any more. Nothing in it is HTTP-shaped: it returns the same `HealthReport`
 * the contract already defines, and rendering it for a screen or for a chat is

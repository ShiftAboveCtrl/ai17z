-- RESEARCH_DONE: the agent looked something up off-platform.
--
-- Fourth time this pattern has come round, and the warning in CLAUDE.md is
-- there because of the previous three: an enum grows in the contract, the code
-- writes the new value, and the CHECK constraint still lists the old set. The
-- write fails at the database and every unit test passes.
--
-- tests/integration/statusConstraints.test.ts writes every value of this enum,
-- so adding one without coming here fails there.
...
ALTER TABLE trace_events
  ADD CONSTRAINT trace_events_type_check
    CHECK (type IN (
      ...
      'QUALITY_SCORED','REPETITION_DETECTED','RESEARCH_DONE'
    ));

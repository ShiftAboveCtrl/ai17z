...
yourself. XBAM never handles your password: it drives a browser that is already
signed in.
...
## Option A — Managed profile (recommended when XBAM owns the browser)

XBAM launches the browser and owns a persistent profile directory. You sign in
once through the UI and the session lives in that profile across restarts.

1. Open the account in XBAM, set **Browser → Managed profile**.
...
XBAM attaches to a browser you started, and never closes it.
...
2. In XBAM, set **Browser → Attach over CDP** and the URL to
   `http://127.0.0.1:9222`.
...
XBAM detaches from a CDP browser when it is done. It does not close it.
...
## What XBAM deliberately does not do
...
What XBAM does instead is use a real browser with a real session you signed into,
